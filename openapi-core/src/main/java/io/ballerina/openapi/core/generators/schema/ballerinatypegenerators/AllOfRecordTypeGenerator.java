    ...
    public TypeDescriptorNode generateTypeDescriptorNode() throws BallerinaOpenApiException {
        ...
        if (allOfSchemas.size() == 1 && allOfSchemas.get(0).get$ref() != null) {
            ...
            if (recordFieldList.size() == 1) {
                List<Schema<?>> right = recordFlist.getRight();
                TypeGenerator typeGenerator = getTypeGenerator(right.get(0), typeName, null);
        ...
        for (Schema allOfSchema : allOfSchemas) {
            if (allOfSchema.get$ref() != null) {
                ...
                if (allOfSchema.getAllOf() != null) {
                    ImmutablePair<List<Node>, List<Schema<?>>> immutablePair = generateAllOfRecordFields(allOfSchema.getAllOf());
